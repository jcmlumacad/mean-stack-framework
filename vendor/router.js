...
module.exports = {
    ...
    update: function (uri, controller, auth) {
        var str = controller.split('@');
        var module = str[0].split('.');
        this.config.app.put(uri, (auth) ? middleware(auth) : this.config.callback, ctrl(str[0], str[1], module[0]));
    },
    destroy: function (uri, controller, auth) {
        var str = controller.split('@');
        var module = str[0].split('.');
        this.config.app.delete(uri, (auth) ? middleware(auth) : this.config.callback, ctrl(str[0], str[1], module[0]));
    },
